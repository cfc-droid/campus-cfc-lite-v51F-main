/* =========================================================
   PEA — EXPORT WORD (HTML COMPATIBLE)
   Archivo: pea_export_word.js
   Rol: exportar HTML compatible con Word
   Fuente: viewModel filtrado (inyectado)
   ========================================================= */

import { generatePEAHash } from "./pea_hash.js";

/**
 * @param {Object} viewModel
 * @param {"LIMPIO"|"LEGAL"} mode
 */
export async function exportPEAWord(viewModel, mode = "LIMPIO") {
  if (!viewModel || !Array.isArray(viewModel.records)) {
    throw new Error("ViewModel inválido para export Word");
  }

  if (!["LIMPIO", "LEGAL"].includes(mode)) {
    throw new Error("Modo Word inválido");
  }

  const exportTime = new Date().toISOString();

  const payload = {
    export_type: `PEA_EXPORT_WORD_${mode}`,
    export_time: exportTime,
    filters: viewModel.filters || {},
    metrics: viewModel.metrics || {},
    records: viewModel.records
  };

  const serialized = JSON.stringify(payload, null, 2);
  const hash = await generatePEAHash(serialized);

  const headerLegal = mode === "LEGAL"
    ? `
      <h1>ANÁLISIS (PEA) — EXPORT LEGAL</h1>
      <p>Fecha de exportación: ${exportTime}</p>
      <p>Hash de integridad (SHA-256): ${hash}</p>
      <hr/>
    `
    : `
      <h1>ANÁLISIS (PEA) — EXPORT LIMPIO</h1>
      <p>Fecha de exportación: ${exportTime}</p>
      <hr/>
    `;

  const html = `
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>PEA Export Word</title>
<style>
body { font-family: Arial, sans-serif; font-size: 12px; }
table { border-collapse: collapse; width: 100%; margin-top: 10px; }
td, th { border: 1px solid #000; padding: 4px; font-size: 11px; }
</style>
</head>
<body>

${headerLegal}

<h2>Filtros aplicados</h2>
<pre>${JSON.stringify(viewModel.filters || {}, null, 2)}</pre>

<h2>Métricas visibles</h2>
<pre>${JSON.stringify(viewModel.metrics || {}, null, 2)}</pre>

<h2>Registros</h2>
<table>
<thead>
<tr>
<th>Fecha</th>
<th>Momento</th>
<th>Pensamiento</th>
<th>Estado</th>
<th>Int.</th>
<th>Acciones</th>
<th>Dirección</th>
<th>Nota</th>
<th>Estado reg.</th>
</tr>
</thead>
<tbody>
${viewModel.records.map(r => `
<tr>
<td>${r.fecha}</td>
<td>${r.momento}</td>
<td>${r.P?.pensamiento_key || ""}</td>
<td>${r.E?.estado_key || ""}</td>
<td>${r.E?.intensidad ?? ""}</td>
<td>${(r.A?.acciones_keys || []).join(", ")}</td>
<td>${r.direccion}</td>
<td>${r.nota_factual || ""}</td>
<td>${r.meta?.estado || ""}</td>
</tr>
`).join("")}
</tbody>
</table>

</body>
</html>
`;

  const blob = new Blob([html], { type: "text/html;charset=utf-8;" });
  const url = URL.createObjectURL(blob);

  return {
    url,
    filename: `PEA_EXPORT_WORD_${mode}_${exportTime}.html`,
    hash
  };
}
